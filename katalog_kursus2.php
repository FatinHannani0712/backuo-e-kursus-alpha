...
$tarikh_mula_dari = isset($_GET['tarikh_mula_dari']) ? $_GET['tarikh_mula_dari'] : '';
$tarikh_mula_hingga = isset($_GET['tarikh_mula_hingga']) ? $_GET['tarikh_mula_hingga'] : '';

// Build query based on filters - IMPROVED ORDERING
$sql = "SELECT k.*, 
               COALESCE(d.jumlah_daftar, 0) as jumlah_sudah_daftar,
               CASE 
                   WHEN k.status_kursus != 'buka_pendaftaran' THEN k.status_kursus
                   WHEN k.pendaftar >= k.kapasiti THEN 'penuh'
                   WHEN k.pendaftar >= (k.kapasiti * 0.8) THEN 'hampir_penuh'
...
// Add date filters
if (!empty($tarikh_mula_dari)) {
    $sql .= " AND k.tarikh_mula >= :tarikh_mula_dari";
    $params[':tarikh_mula_dari'] = $tarikh_mula_dari;
}

if (!empty($tarikh_mula_hingga)) {
    $sql .= " AND k.tarikh_mula <= :tarikh_mula_hingga";
    $params[':tarikh_mula_hingga'] = $tarikh_mula_hingga;
}

// IMPROVED ORDERING: Open courses first, then by category
$sql .= " ORDER BY 
          CASE 
...
// Function to get status badge class - IMPROVED
function getStatusClass($status) {
    switch($status) {
        ...
        case 'hampir_penuh': return 'status-nearfull'; // NEW
        default: return 'status-upcoming';
    }
}

// Function to get status text - IMPROVED
function getStatusText($status) {
    switch($status) {
        case 'buka_pendaftaran': return 'Buka Pendaftaran';
        case 'akan_datang': return 'Akan Datang';
        case 'penuh': return 'Penuh';
        case 'hampir_penuh': return 'Hampir Penuh'; // CLEARER than "Senarai Tunggu"
...
<head>
  ...
  <style>
    :root {
      /* APM OFFICIAL COLORS */
      --apm-orange: #FF6B00;      /* Main APM Orange */
      --apm-dark-orange: #E55A00;  /* Darker orange for hovers */
      --apm-red: #DC143C;          /* APM Red accent */
      --apm-dark-blue: #1B365D;    /* APM Dark Blue */
      --apm-navy: #0F2A44;         /* Darker navy */
      --apm-white: #FFFFFF;
    ...
    .register-btn { 
      ...
      color: white; 
      border: 1px solid var(--apm-orange); 
    }
    .register-btn:hover { 
      background-color: var(--apm-dark-orange); 
      transform: translateY(-2px); 
    }
    .profile-btn { 
    ...
    .stat-card {
      ...
      border-left: 4px solid var(--apm-orange);
    ...
    /* Search & Filter Section - APM COLORS */
    .search-filter-section {
      ...
      margin-bottom: 2rem;
      box-shadow: 0 4px 8px rgba(0,0,0,0.05);
      border-left: 6px solid var(--apm-dark-blue);
    }
    ...
    .search-bar input:focus, .date-input:focus {
      outline: none;
    ...
    .btn-secondary-custom {
      background: white;
      color: var(--apm-dark-blue);
      border: 2px solid var(--apm-dark-blue);
    }
...
<div class="container">
  ...
  <div class="health-notice">
    <div class="health-notice-content">
      <h5>
        ...
        Keperluan Borang Kesihatan
      </h5>
      <p>
        <strong>PENTING:</strong> Semua peserta yang ingin mendaftar kursus dikehendaki melengkapkan 
        <strong>Borang Pengakuan Kesihatan</strong> terlebih dahulu. Borang ini adalah syarat wajib 
        sebelum permohonan anda dapat diproses.
      </p>
      <?php if ($isLoggedIn): ?>
        <a href="borang_kesihatan.php" class="btn btn-warning btn-sm">
          <i class="fas fa-file-medical"></i> Lengkapkan Borang Kesihatan
        </a>
      <?php else: ?>
        <span class="text-muted">Sila log masuk untuk mengakses borang kesihatan</span>
      <?php endif; ?>
    </div>
  </div>
      ...
      <div class="col-12">
        ...
        <form method="GET" action="">
          <div class="row">
            <div class="col-md-12">
              <div class="search-bar">
                <input type="text" name="search" class="form-control" placeholder="Cari kursus mengikut nama atau penerangan..." value="<?php echo htmlspecialchars($search); ?>">
          ...
          <div class="date-filter-section">
            <h6 class="mb-3"><i class="fas fa-calendar-alt me-2"></i>Carian Berdasarkan Tarikh</h6>
            <div class="row">
              <div class="col-md-6 mb-3">
                <label class="form-label small">Tarikh Mula Dari:</label>
                <input type="date" name="tarikh_mula_dari" class="date-input" value="<?php echo htmlspecialchars($tarikh_mula_dari); ?>">
              </div>
              <div class="col-md-6 mb-3">
                <label class="form-label small">Tarikh Mula Hingga:</label>
                <input type="date" name="tarikh_mula_hingga" class="date-input" value="<?php echo htmlspecialchars($tarikh_mula_hingga); ?>">
              </div>
            </div>
          </div>
          
          <div class="filter-tabs">
            <a href="?kategori=semua<?php echo !empty($search) ? '&search='.urlencode($search) : ''; ?><?php echo !empty($tarikh_mula_dari) ? '&tarikh_mula_dari='.urlencode($tarikh_mula_dari) : ''; ?><?php echo !empty($tarikh_mula_hingga) ? '&tarikh_mula_hingga='.urlencode($tarikh_mula_hingga) : ''; ?>" 
               class="filter-tab <?php echo $kategori_filter == 'semua' ? 'active' : ''; ?>">
              <i class="fas fa-th-large me-1"></i> Semua Kursus
            </a>
            
            <?php foreach ($kategori_list as $kat): ?>
            <a href="?kategori=<?php echo urlencode($kat['kategori']); ?><?php echo !empty($search) ? '&search='.urlencode($search) : ''; ?><?php echo !empty($tarikh_mula_dari) ? '&tarikh_mula_dari='.urlencode($tarikh_mula_dari) : ''; ?><?php echo !empty($tarikh_mula_hingga) ? '&tarikh_mula_hingga='.urlencode($tarikh_mula_hingga) : ''; ?>" 
      ...
      <div class="course-footer">
        ...
        <?php else: ?>
          ...
          <?php elseif ($kursus['status_display'] == 'penuh'): ?>
            ...
            <?php endif; ?>
          <?php elseif ($kursus['status_display'] == 'hampir_penuh'): ?>
            <?php if ($isLoggedIn): ?>
              <a href="daftar_kursus.php?id=<?php echo $kursus['kursus_id']; ?>" class="btn-custom btn-warning-custom">
  ...
  <!-- COMBINED INFORMATION SECTION - IMPROVED -->
  <div class="row mt-5">
    <div class="col-md-4 mb-4">
      <div class="course-card">
        ...
        <h4 class="course-title">Hubungi Kami & Sokongan</h4>
        <p class="course-description">
          Perlukan bantuan dengan permohonan kursus atau mempunyai pertanyaan? Hubungi pasukan sokongan APM ALPHA:
        </p>
        <ul class="list-unstyled small mb-3">
          <li class="mb-1"><i class="fas fa-phone text-primary me-2"></i><strong>Tel:</strong> 03-8064 2222</li>
          <li class="mb-1"><i class="fas fa-envelope text-primary me-2"></i><strong>Email:</strong> info@alpha.gov.my</li>
          <li class="mb-1"><i class="fas fa-globe text-primary me-2"></i><strong>Web:</strong> www.alpha.gov.my</li>
        </ul>
        <div class="course-footer">
    ...
    <div class="col-md-4 mb-4">
      <div class="course-card">
        ...
        <h4 class="course-title">Maklumat Pendaftaran</h4>
        <p class="course-description">
          Semua kursus dijalankan di ALPHA dengan kemudahan terkini. Pastikan profil dan borang kesihatan lengkap sebelum memohon.
        </p>
        <ul class="list-unstyled small mb-3">
          <li class="mb-1"><i class="fas fa-check text-success me-2"></i>Kursus percuma untuk kakitangan kerajaan</li>
          <li class="mb-1"><i class="fas fa-check text-success me-2"></i>Sijil diiktiraf akan diberikan</li>
          <li class="mb-1"><i class="fas fa-check text-success me-2"></i>Kemudahan penginapan & makanan</li>
        </ul>
        <div class="course-footer">
    ...
    <div class="col-md-4 mb-4">
      <div class="course-card">
        ...
        <h4 class="course-title">Senarai Permohonan</h4>
        <p class="course-description">
          Lihat status semua permohonan kursus yang telah anda buat, termasuk yang sedang diproses, diterima atau ditolak.
        </p>
        <?php if ($isLoggedIn): ?>
          <div class="course-footer">
            <a href="dashboard.php" class="btn-custom btn-primary-custom w-100">
        ...
        <?php else: ?>
          <div class="alert alert-info border-0 rounded-3 small">
            ...
            Sila log masuk untuk melihat senarai permohonan anda.
          </div>
          <div class="course-footer">
            <a href="login.php" class="btn-custom btn-primary-custom w-100">
              <i class="fas fa-sign-in-alt"></i>
              Log Masuk
            </a>
          </div>
...
<script>
  ...
  document.querySelectorAll('.course-card').forEach(card => {
    card.addEventListener('mouseenter', function() {
      this.style.borderLeftColor = 'var(--apm-orange)';
    });
    
    card.addEventListener('mouseleave', function() {
      this.style.borderLeftColor = 'var(--apm-dark-blue)';
    });
  });
  ...
  function clearFilters() {
    window.location.href = 'katalog_kursus.php';
  }

  // Add clear filter button if any filters are active
  document.addEventListener('DOMContentLoaded', function() {
    const urlParams = new URLSearchParams(window.location.search);
    const hasFilters = urlParams.get('search') || urlParams.get('kategori') !== 'semua' || 
                      urlParams.get('tarikh_mula_dari') || urlParams.get('tarikh_mula_hingga');
    
    if (hasFilters) {
      const clearBtn = document.createElement('button');
      clearBtn.innerHTML = '<i class="fas fa-times"></i> Kosongkan Penapis';
      clearBtn.className = 'btn btn-outline-secondary btn-sm mt-2';
      clearBtn.onclick = clearFilters;
      document.querySelector('.filter-tabs').appendChild(clearBtn);
    }
  });
</script>
