...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($exists) {
            // UPDATE
            $sqlUpdate = " UPDATE maklumat_diri SET
                    nama = ?,
                    no_ahli = ?,
                    status = ?,
                    jantina = ?,
            ...
            // INSERT
            $sqlInsert = "INSERT INTO maklumat_diri
                    (user_id, nama, no_ahli,status, jantina, umur, tarikh_keahlian, jenis_keahlian, pangkat,
                     ...
                     nama_kecemasan, alamat_kecemasan, telefon_kecemasan, pengesahan, tarikh_akuan)
                VALUES
                    (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
      ...
      <ul class="nav nav-tabs mb-4" id="profileTabs" role="tablist">
        ...
        <li class="nav-item" role="presentation">
          <button class="nav-link <?= isset($errors['pengakuan']) || isset($errors['no_tel_waris']) ? 'tab-error' : '' ?>" id="kin-tab" data-bs-toggle="tab" data-bs-target="#kin" type="button" role="tab">
            <i class="fas fa-users me-2"></i>Waris & Pengesahan
          </button>
        </li>
      </ul>
                ...
                <div class="mb-3">
                  ...
                  <div class="input-group">
                    ...
                    <select class="form-control" id="status" name="status">
                      ...
                      <option value="Balu"     <?= (($userData['status'] ?? '')==='Balu')?'selected':''; ?>>Balu</option>
                    </select>
                  </div>
                </div>
              </div>
          ...
          <div class="tab-pane fade" id="kin" role="tabpanel">
            ...
            <div class="mb-3 form-check">
              <input class="form-check-input <?= isset($errors['pengakuan']) ? 'is-invalid' : '' ?>" 
                    ...
                    <?= (($userData['pengesahan'] ?? '') === 'YA') ? 'checked' : ''; ?>>
              ...
              <div class="mb-3">
                ...
                <div class="input-group">
                  ...
                  <input type="text" class="form-control" id="tarikh_akuan_display" 
                        value="<?= !empty($userData['tarikh_akuan']) ? date('d/m/Y', strtotime($userData['tarikh_akuan'])) : date('d/m/Y'); ?>" 
                        readonly style="background-color: #f8f9fa;">
                  <input type="hidden" id="tarikh_akuan" name="tarikh_akuan" 
                        value="<?= htmlspecialchars($userData['tarikh_akuan'] ?? date('Y-m-d')); ?>">
                </div>
                <div class="form-text">Tarikh semasa akan digunakan secara automatik apabila pengesahan ditandakan.</div>
              </div>
              ...
              <div class="mb-3">
                ...
                <div class="alert alert-info py-2">
                  ...
                  <span id="pengesahan_status">
                    <?php if (($userData['pengesahan'] ?? '') === 'YA'): ?>
                      Telah disahkan pada <?= date('d/m/Y', strtotime($userData['tarikh_akuan'])); ?>
                    <?php else: ?>
                      Belum disahkan
                    <?php endif; ?>
                  </span>
...
<script>
  ...
  // Enhanced date handling for pengesahan
document.addEventListener('DOMContentLoaded', function() {
  const pengakuanCheckbox = document.getElementById('pengakuan');
  const tarikhAkuanInput = document.getElementById('tarikh_akuan');
  const tarikhAkuanDisplay = document.getElementById('tarikh_akuan_display');
  const pengesahanStatus = document.getElementById('pengesahan_status');
  
  // Set initial state
  updateDateFieldState();
  
  // Handle checkbox change
  pengakuanCheckbox.addEventListener('change', function() {
    if (this.checked) {
      // Show confirmation dialog
      if (confirm('Adakah anda pasti untuk mengesahkan maklumat ini? Tarikh semasa akan digunakan dan tidak boleh diubah.')) {
        const today = new Date();
        const formattedDate = today.toISOString().split('T')[0];
        const displayDate = formatDateForDisplay(today);
        
        tarikhAkuanInput.value = formattedDate;
        tarikhAkuanDisplay.value = displayDate;
        pengesahanStatus.textContent = 'Telah disahkan pada ' + displayDate;
        
        // Show success message
        showTemporaryMessage('Maklumat telah disahkan dengan tarikh semasa.', 'success');
      } else {
      ...
      if (confirm('Dengan membatalkan pengesahan, status maklumat anda akan kembali kepada belum disahkan. Teruskan?')) {
        updateDateFieldState();
      } else {
  ...
  function updateDateFieldState() {
    if (pengakuanCheckbox.checked) {
      // If no date is set, use today's date
      if (!tarikhAkuanInput.value) {
        const today = new Date();
        tarikhAkuanInput.value = today.toISOString().split('T')[0];
        tarikhAkuanDisplay.value = formatDateForDisplay(today);
      }
      pengesahanStatus.textContent = 'Telah disahkan pada ' + tarikhAkuanDisplay.value;
    } else {
      pengesahanStatus.textContent = 'Belum disahkan';
    }
  }
  ...
  function showTemporaryMessage(message, type) {
    ...
    // Insert after the pengesahan section
    const pengesahanSection = document.querySelector('.form-section:last-of-type');
    pengesahanSection.parentNode.insertBefore(alertDiv, pengesahanSection.nextSibling);
